...
class HomeTravelTab extends Component {
    ...
    handleTravelPolicy = e => {
        ...
        var requestParam = {};
      if (this.location.current.value === null || this.location.current.value === "") {
            formIsValid = false;
            ...
            AxiosTrans({
                ...
                .then(res => {
                    if (res.data.token) {
                        redirectUrl = redirectUrl + "/travel/display/key/"+res.data.key+"/token/" + res.data.token;
    ...
    render() {
        let sbiHomeData = localStorage.getItem("phrases") ? JSON.parse(localStorage.getItem("phrases")) : null;
        let lang_name = localStorage.getItem("lang_name") ? localStorage.getItem("lang_name") : 'en';
        let buy, claim, Select_Gender, Male, Female, Go, Father_In_Law,
              Select_Location, Worldwide_including, Worldwide_excluding, Father,
             Add_Family_Members,Mother, Mother_In_Law,
            Self, Spouse, Child_1, Child_2,want_to_insure,select_insured_member,to_travel,up_to_4,travelling_from_india;
